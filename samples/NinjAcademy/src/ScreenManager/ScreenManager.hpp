...
// NinjAcademy sample). Tombstoning (SerializeState/DeserializeState) and the
// debug TraceScreens() helper are dropped, matching this project's
// established precedent — see missing.md.
...
public:
    ...
    void RemoveScreen(GameScreen* screen) {
        ...
        // zero here. A screen like GameplayScreen owns dozens of
        ...
        // synchronously mid-iteration. (See NEXT.md "pattern to watch for",
        // first hit and fixed in HoneycombRush's ScreenManager.)
...
inline void GameScreen::ExitScreen() {
    if (transitionOffTime_.getTotalMillisecondsProperty() <= 0.0) {
        ...
        // Mark the screen as a pop-up, so as not to block lower screens in the one cycle where it vanishes.
        isPopup_ = true;
    } else {
        isExiting_ = true;
    }
