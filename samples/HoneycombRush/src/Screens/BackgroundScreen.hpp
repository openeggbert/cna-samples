...
// Simple full-screen background image. Port of Screens/BackgroundScreen.cs.
class BackgroundScreen : public GameScreen {
public:
    explicit BackgroundScreen(std::string backgroundName) : backgroundName_(std::move(backgroundName)) {
    ...
    void LoadContent() override {
        background_.emplace(
            GetScreenManager()->getGameProperty().getContentProperty().Load<Texture2D>("Textures/Backgrounds/" + backgroundName_));
    }
