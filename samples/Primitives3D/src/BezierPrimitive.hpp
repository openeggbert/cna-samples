...
{
    class BezierPrimitive : public GeometricPrimitive
    {
    protected:
        void CreatePatchIndices(int tessellation, bool isMirrored)
        {
            ...
            for (int i = 0; i < tessellation; ++i)
            {
                for (int j = 0; j < tessellation; ++j)
                {
                    ...
                    if (isMirrored)
                        std::reverse(std::begin(idx), std::end(idx));
        ...
        void CreatePatchVertices(const Vector3 patch[16], int tessellation, bool isMirrored)
        {
            for (int i = 0; i <= tessellation; ++i)
            {
                float ti = static_cast<float>(i) / tessellation;

                for (int j = 0; j <= tessellation; ++j)
                {
                    float tj = static_cast<float>(j) / tessellation;
    ...
    private:
        ...
        static Vector3 BezierTangent(Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4, float t)
        {
            Vector3 result(
                BezierTangent(p1.X, p2.X, p3.X, p4.X, t),
                BezierTangent(p1.Y, p2.Y, p3.Y, p4.Y, t),
                BezierTangent(p1.Z, p2.Z, p3.Z, p4.Z, t));
            result.Normalize();
            return result;
        }
    };
