...
// Ported from PickingSample.Cursor (Cursor.cs). The original supports Xbox/Windows/
// Windows-Phone input branches (#if XBOX / WINDOWS / WINDOWS_PHONE); this desktop port
// keeps only the Windows (mouse) branch, matching the .htm's own control scheme ("Mouse"
// moves the cursor on Windows) -- see missing.md.
