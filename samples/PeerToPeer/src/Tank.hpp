...
// Direct port of Tank.cs. Confirmed byte-identical to ClientServerSample's own Tank.cs
// (the only difference is the surrounding C# namespace) via `diff`/`md5sum` -- reused
// ClientServerSample's already-converted Content/Tank.png, Turret.png, font.font.json,
// font.png directly instead of reconverting, per this repo's own "don't regenerate
// existing assets unless there's a confirmed bug" guidance (NetworkPrediction already
// established this same reuse for the same source files).
