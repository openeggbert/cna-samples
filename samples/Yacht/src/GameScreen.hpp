...
public:
    ...
    // Gestures this screen wants TouchPanel to detect. Screens should keep
    // this as narrow as possible, mirroring the original's own advice
    // (previously WINDOWS_PHONE-only; un-#ifdef'd since CNA's touch is real).
    GestureType EnabledGestures() const { return enabledGestures_; }
    void setEnabledGestures(GestureType value) {
        enabledGestures_ = value;

        // The screen manager handles this during screen changes, but if this
        // screen is already active we must update TouchPanel ourselves.
