...
// Helper for reading input from keyboard, gamepad and mouse. Tracks both the
// current and previous state and implements high-level query methods such as
// "move up through the menu" or "pause the game".
//
// The XNA original (a Windows Phone game) drove everything via TouchPanel
// gestures; on desktop CNA we adapt drag/tap to the mouse (see missing.md).
class InputState {
public:
    ...
    // Reads the latest state of the keyboard, gamepad and mouse.
    void Update() {
    ...
    // ---- Mouse helpers (desktop substitute for touch gestures) ----
    ...
    // The left mouse button was just pressed this frame.
    bool IsNewLeftMousePress() const {
        return CurrentMouseState.getLeftButtonProperty() == ButtonState::Pressed &&
               LastMouseState.getLeftButtonProperty() == ButtonState::Released;
    }

    // The left mouse button was just released this frame.
    ...
    // A "tap": a fresh mouse click or a keyboard/gamepad select. Used by the
    // screens that say "Tap to continue" in the touch original.
    bool IsTap(std::optional<PlayerIndex> controllingPlayer) {
        PlayerIndex playerIndex;
        return IsNewLeftMousePress() || IsMenuSelect(controllingPlayer, playerIndex);
    }
};
