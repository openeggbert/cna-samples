...
// InputState.hpp — C++ port of ScreenManager/InputState.cs (XNA 4.0 MarbleMaze
// sample, vanilla "GameStateManagement" library). Helper for reading input from
// keyboard, gamepad, and touch. Tracks current/previous device state and
// implements high-level query methods such as "menu select" or "pause the game".
//
// CNA addition (see missing.md): this desktop has no touchscreen, so a left
// mouse-click's rising edge synthesizes a Tap gesture (this repo's established
// touch/mouse-fallback convention, see NEXT.md section 6 and
// samples/HoneycombRush/src/ScreenManager/InputState.hpp) -- every screen that
// only checks input.Gestures for a Tap (all of this sample's menu/loading/
